...
// Tambah lokasi/departemen
if (isset($_POST['add_location'])) {
    $name = sanitize($_POST['name']);
    $details = sanitize($_POST['details']);

    $stmt = $conn->prepare("INSERT INTO departments (name, details) VALUES (?, ?)");
    $stmt->bind_param("ss", $name, $details);
    $stmt->execute();
    redirect('locations.php');
}

// Hapus lokasi/departemen
...
if (isset($_GET['edit'])) {
    $id = (int)$_GET['edit'];
    $res = $conn->query("SELECT * FROM departments WHERE id = $id LIMIT 1");
    $edit_location = $res->fetch_assoc();
}

// Update lokasi/departemen
if (isset($_POST['update_location'])) {
    $id = (int)$_POST['id'];
    $name = sanitize($_POST['name']);
    $details = sanitize($_POST['details']);

    $stmt = $conn->prepare("UPDATE departments SET name = ?, details = ? WHERE id = ?");
...
<html>
<head>
    <title>Manajemen Lokasi / Departemen - Insidentia</title>
    ...
    <style>
        ...
        /* Specific styling for the badge in location table */
        .table-container .table tbody tr td:first-child .badge {
            background-color: #ffb6c1 !important; /* Lighter pink for the location name badge */
            color: #333;
            font-weight: normal;
        }
    ...
    <div class="row">
        ...
        <div class="col-md-3 sidebar p-3">
            ...
            <nav class="nav flex-column">
                ...
                <a href="locations.php" class="nav-link fw-bold active"><i class="bi bi-geo-alt me-2"></i> Lokasi</a>
        ...
        <div class="col-md-9 p-4">
            <h2 class="text-center mb-4 section-title"><i class="bi bi-geo-alt me-2"></i>Manajemen Lokasi / Departemen</h2>

            <div class="row g-4">
                <!-- Form Section -->
                <div class="col-md-4">
                    <div class="card-form shadow">
                        <h5 class="text-white"><?= $edit_location ? 'Edit Lokasi / Departemen' : 'Tambah Lokasi / Departemen' ?></h5>
                        <form method="post">
                            <input type="hidden" name="id" value="<?= $edit_location['id'] ?? '' ?>">
                            <div class="mb-3">
                                <label class="form-label">Nama Lokasi / Departemen</label>
                                <input type="text" name="name" class="form-control" required value="<?= $edit_location['name'] ?? '' ?>">
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Detail</label>
                                <textarea name="details" class="form-control" rows="3"><?= $edit_location['details'] ?? '' ?></textarea>
                            </div>
                            <button type="submit" name="<?= $edit_location ? 'update_location' : 'add_location' ?>" class="btn btn-pink w-100">
                                <?= $edit_location ? 'Update' : 'Tambah' ?>
                            </button>
                        </form>
                    </div>
                </div>
                ...
                <div class="col-md-8">
                    <div class="table-container">
                        <h5 class="fw-bold text-danger mb-3"><i class="bi bi-list-ul me-2"></i>Daftar Lokasi / Departemen</h5>
                                    ...
                                    <?php while($row = $result->fetch_assoc()): ?>
                                        <tr>
                                            ...
                                            <td class="text-center">
                                                <a href="locations.php?edit=<?= $row['id'] ?>" class="btn btn-sm btn-warning me-1" data-bs-toggle="tooltip" title="Edit"><i class="bi bi-pencil-fill"></i></a>
                                                <a href="locations.php?delete=<?= $row['id'] ?>" class="btn btn-sm btn-danger" data-bs-toggle="tooltip" title="Hapus" onclick="return confirm('Hapus lokasi ini?')"><i class="bi bi-trash-fill"></i></a>
                                            </td>
                                        </tr>
                                    <?php endwhile; ?>
                                </tbody>
